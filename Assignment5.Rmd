...
t1<-t.test(thriller$runtime,conf.level=0.99)
...
sigma<-sd(thriller$runtime)
n<-length(thriller$runtime)

z_a2<-qnorm(0.995)
error<-z_a2*sigma/sqrt(n)
...
```{r}
#WRITE YOUR CODE HERE
s2<-var(thriller$runtime)
h0<-500
df<-length(thriller$runtime)-1
chisq<-df*s2/h0

chi_low<-qchisq(0.025,df)
chi_high<-qchisq(0.975,df)

print(chisq)
print(c(chi_low,chi_high))
...
```{r}
#WRITE YOUR CODE HERE
hist(thriller$runtime,
     main="Distribution of Thriller Runtime",
     xlab="Runtime")
...
mean_profits<-tapply(movies$profit,
                     movies$vote_average_rounded,mean,na.rm=TRUE)
print(mean_profits)
...
for(i in 1:length(years)){
  profits<-movies$profit[movies$release_year==years[i]]
  profits<-profits[!is.na(profits)]
  t1<-t.test(profits)
  lower[i]<-t1$conf.int[1]
  upper[i]<-t1$conf.int[2]
}

plot(years,
     mean_profits,
     main="Mean Movie Profit by Year",
     xlab="Release Year",
     ylab="Mean Profit")
